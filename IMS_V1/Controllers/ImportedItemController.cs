...
namespace IMS_V1.Controllers
{
    public class ImportedItemController : Controller
    {
        ...
        public ActionResult ImportItems(string currentFilter, string SearchString, int? page)
        {
            ...
            var items = from r in
                            db.ImportedItems where r.CreatedBy == userid && (r.Imported == false || r.Imported == null)
                        select r;        
        ...
        public ActionResult CreateItem([Bind(Exclude = "Item_id")]Item model)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    SaveItemAttributesCreateDesc(importedItem);
        ...
        protected void SaveItemAttributesCreateDesc(ImportedItem impItem)
        {
            Item item = db.Items.Find(impItem.Item_id);
            if (item == null) return;
            var result1 = db.Database.ExecuteSqlCommand("UpdateItemDescription @Item_Id,@NewDescription,@UserId", new SqlParameter("@Item_Id", item.Item_id),
                                                                                                new SqlParameter("@NewDescription", impItem.Item_Description),
        ...
        protected void InsertItemAttributes(ImportedItem impItem)
        {
            if (impItem.CategoryClass_Id == 1 || impItem.CategoryClass_Id == 2)
                InsertGunsAttributes(impItem);
            else
                InsertAmmoAttributes(impItem);
        }

        protected void InsertGunsAttributes(ImportedItem impItem)
        {
            int userId = int.Parse(Session.Contents["UserId"].ToString());
            ...
            if (impItem.ModelManuValue != null && impItem.ModelManuValue.Trim().Length > 0)
                at.ActualAttributeValue = impItem.ModelManuValue;
            db.ItemAttributes.Add(at);
            db.SaveChanges();
            ...
            if (impItem.CategoryClass_Id == 2)
            {
                ...
                if (impItem.MiscManuValue != null && impItem.MiscManuValue.Trim().Length > 0)
                    at.ActualAttributeValue = impItem.MiscManuValue;
                db.ItemAttributes.Add(at);
                db.SaveChanges();
            }
        }
        ...
        protected void SaveAmmos(HttpPostedFileBase file, string path)
        {
            ...
            try
            {
                ...
                using (IMS_V1Entities db1 = new IMS_V1Entities())
                {
                    ...
                    while (((Excel.Range)range.Cells[i, 1]).Text.Trim().Length > 0)
                    {
                        impItem.Buyer = buyer;
                        ...
                        for (int k = 1; k < 59; k++)
                        {
                            ...
                            switch (k)
                            {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    string abbr = ((Excel.Range)range.Cells[i, k]).Text.Trim() + ((Excel.Range)range.Cells[i, k + 1]).Text.Trim() + ((Excel.Range)range.Cells[i, k + 2]).Text.Trim();
                                    if (abbr.Length != 3)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "Item abbr is not 3 letters");
                                        breakTrue = true;
                                    }
                                    else
                                    {
                                        impItem.ManufacturerLogo_Id = GetManufacturerLogo_Id(db, abbr);
                                        if (impItem.ManufacturerLogo_Id == 0)
                                        {
                                            itemMessage = LogMessage(impItem, itemMessage, "abbreviation is not valid");
                                            breakTrue = true;
                                        }
                                    }
                                    break;
                                case 6:
                                    impItem.CategoryClass_Id = GetCategoryClass_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2));
                                    if (impItem.CategoryClass_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, " category  is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 7:
                                    impItem.SubClassCode_Id = GetSubClass_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2), "0" + impItem.CategoryClass_Id);
                                    if (impItem.SubClassCode_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, " subclass is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 8:
                                    impItem.FineLineCode_Id = GetFineLineClass_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2), ((Excel.Range)range.Cells[i, k - 2]).Text.Trim().Substring(0, 2), ((Excel.Range)range.Cells[i, k - 1]).Text.Trim().Substring(0, 2));
                                    if (impItem.FineLineCode_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, " fineline is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 9:
                                ...
                                case 11:
                                    ...
                                    {
                                        impItem.UM_Id = 0;
                                        itemMessage = LogMessage(impItem, itemMessage, "unit of measure is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 12:
                                    ...
                                    {
                                        impItem.VdUM_id = 0;
                                        itemMessage = LogMessage(impItem, itemMessage, " vendor unit of measure is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 13:
                                    string vsCost = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.VICost = CheckAndAsignValue(vsCost);
                                    break;
                                case 14:
                                ...
                                case 15:
                                    ...
                                    impItem.WholeSaleMTPPrice = CheckAndAsignValue(wholeSaleMTPPrice);                                   
                                    break;
                                case 16:
                                ...
                                case 17:
                                    ...
                                    impItem.MinAdvertisePrice = CheckAndAsignValue(minAdvPrice);                                  
                                    break;
                                case 18:
                                    string msrpPrice = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.MSRP = CheckAndAsignValue(msrpPrice);
                                    break;
                                case 20:
                                    string level1Price = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.Level1 = CheckAndAsignValue(level1Price);
                                    break;
                                case 22:
                                    string level2Price = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.Level2 = CheckAndAsignValue(level2Price);
                                    break;
                                case 24:
                                    string level3Price = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.Level3 = CheckAndAsignValue(level3Price);
                                    break;
                                case 26:
                                    string level4Price = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.Level4 = CheckAndAsignValue(level4Price);
                                    break;
                                case 28:
                                    string level5Price = ((Excel.Range)range.Cells[i, k]).Text;
                                    impItem.Level5 = CheckAndAsignValue(level5Price);
                                    break;
                                case 30:
                                ...
                                case 31:
                                    ...
                                    impItem.Qty_BreakPrice = CheckAndAsignValue(Qty_Break);                                   
                                    break;
                                case 32:
                                    impItem.CatWebCode_Id = GetCatWebCode_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 3));
                                    if (impItem.CatWebCode_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "cat web code is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 33:
                                    ...
                                    break;
                                case 34:
                                    impItem.Freight_Id = GetFreight_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2));
                                    if (impItem.Freight_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "freight is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 35:
                                    impItem.ABC_Id = GetABC_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 1));
                                    if (impItem.ABC_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "ABC code is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 36:
                                    string std = ((Excel.Range)range.Cells[i, k]).Text;
                                    int num;
                                    if (int.TryParse(std, out num))
                                    {
                                        impItem.STD = num;
                                    }
                                    else
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "std is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 37:
                                ...
                                case 49:
                                    ...
                                    break;
                                case 50:
                                    impItem.BullettTypeId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 27);
                                    if (impItem.BullettTypeId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "bullett type is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 51:
                                    impItem.CaliberId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 5);
                                    if (impItem.CaliberId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "caliber is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 52:
                                    impItem.CountPerBoxId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 28);
                                    if (impItem.CountPerBoxId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "count per box is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 53:
                                    impItem.GrainWeightId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 10);
                                    if (impItem.GrainWeightId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "grain weight is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 54:
                                    impItem.FamilyNameId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 39);
                                    if (impItem.FamilyNameId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "family name is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 55:
                                    impItem.CountPerCaseId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 59);
                                    if (impItem.CountPerCaseId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "count per case is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 56:
                                    impItem.MiscId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 23);
                                    if (impItem.MiscId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "misc is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 57:
                                    impItem.FeetPerSecondId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 49);
                                    if (impItem.FeetPerSecondId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "feet per second is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 58:
                                    impItem.MiscManuValue = ((Excel.Range)range.Cells[i, k]).Text;
                                    break;
                            }
                        }

                        if (goodForImport && MFG_NumberInIMS(db1, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " Manufacturer number " + impItem.MFG_Number + " is in IMS already for manufacturer " + GetManufacturerName(db, impItem));
                            goodForImport = false;
                        }
                        if (goodForImport && MFG_NumberInImportedTable(db1, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " Manufacturer number " + impItem.MFG_Number + " is in imported table already for manufacturer " + GetManufacturerName(db, impItem));
                            goodForImport = false;
                        }
                        if (goodForImport && UpcInIMS(db1, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " UPC " + impItem.UPC + " is in IMS already");
                            goodForImport = false;
                        }
                        if (goodForImport && UpcInImportedTable(db1, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " UPC " + impItem.UPC + " is in imported table already");
                            goodForImport = false;
                        }
                        if (goodForImport)
                        {
                            ...
                            db1.SaveChanges();
                            itemMessage = LogMessage(impItem, itemMessage, " saved.");
                        }
                        else
                        {
            ...
            try
            {
                ...
                using (IMS_V1Entities db = new IMS_V1Entities())
                {
                    ...
                    while (((Excel.Range)range.Cells[i, 1]).Text.Trim().Length > 0)
                    {
                        ...
                        for (int k = 1; k < 56; k++)  
                        {
                            switch (k)
                            {
                                ...
                                case 6:
                                    ...
                                    break;
                                case 7:
                                    if (impItem.CategoryClass_Id < 10)
                                        impItem.SubClassCode_Id = GetSubClass_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2), "0" + impItem.CategoryClass_Id);
                                    else
                                        impItem.SubClassCode_Id = GetSubClass_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim().Substring(0, 2), impItem.CategoryClass_Id.ToString());
                                    if (impItem.SubClassCode_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, " subclass is not valid");
                                        breakTrue = true;
                                ...
                                case 9:
                                    ...
                                    break;
                                case 10:
                                    impItem.FFLType_Id = GetFFLType_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim());
                                    if (impItem.FFLType_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, " FFL type is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 11:
                                ...
                                case 41:
                                    ...
                                    if (impItem.ABC_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "ABC is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 42:
                        ...
                        if (goodForImport && MFG_NumberInIMS(db, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " Manufacturer number " + impItem.MFG_Number + " is in IMS already for manufacturer " + GetManufacturerName(db, impItem));
                            goodForImport = false;
                        }
                        if (goodForImport && MFG_NumberInImportedTable(db, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " Manufacturer number " + impItem.MFG_Number + " is in imported table already for manufacturer " + GetManufacturerName(db, impItem));
                            goodForImport = false;
                        }
                        if (goodForImport && UpcInIMS(db, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " UPC " + impItem.UPC + " is in IMS already");
                            goodForImport = false;
                        }
                        if (goodForImport && UpcInImportedTable(db, impItem))
                        {
                            itemMessage = LogMessage(impItem, itemMessage, " UPC " + impItem.UPC + " is in imported table already");
                            goodForImport = false;
                        }
                        ...
                        for (int k = 1; k < 51; k++) 
                        {
                            switch (k)
                            {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    string abbr = ((Excel.Range)range.Cells[i, k]).Text.Trim() + ((Excel.Range)range.Cells[i, k + 1]).Text.Trim() + ((Excel.Range)range.Cells[i, k + 2]).Text.Trim() + ((Excel.Range)range.Cells[i, k + 3]).Text.Trim();
                                    if (abbr.Length != 4)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "Item abbr is not 4 letters");
                                        breakTrue = true;
                                    }
                                    else
                                    {
            ...
            try
            {
                ...
                using (IMS_V1Entities db = new IMS_V1Entities())
                {
                    ...
                    while (((Excel.Range)range.Cells[i, 1]).Text.Trim().Length > 0)
                    {
                        ...
                        bool goodForImport = true;
                        bool breakTrue = false;
                        for (int k = 1; k < 65; k++) //long gun 61, hand gun 62, ammo 63
                        {
                            if (breakTrue)
                            {
                                goodForImport = false;   
                            ...
                            switch (k)
                            {
                                ...
                                case 10:
                                    ...
                                    if (impItem.FFLType_Id == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "FFLType is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 11:
                                ...
                                case 55:
                                    ...
                                    break;
                                case 56:
                                    impItem.ActionId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 3);
                                    if (impItem.ActionId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "action is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 57:
                                    ...
                                    break;
                                case 58:
                                    impItem.CapacityId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 6);
                                    if (impItem.CapacityId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "capacity is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 59:
                                    impItem.FinishId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 7);
                                    if (impItem.FinishId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "finish is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 60:
                                    impItem.BarrelLengthId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 12);
                                    if (impItem.BarrelLengthId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "barrel length is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 61:
                                    impItem.ModelId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 18);
                                    if (impItem.ModelId == 0)
                                    {
                                        itemMessage = LogMessage(impItem, itemMessage, "model is not valid");
                                        breakTrue = true;
                                    }
                                    break;
                                case 62:
                                    if (impItem.CategoryClass_Id == 1 && ((Excel.Range)range.Cells[i, k]).Text.Trim().Length > 0)
                                    {
                                        impItem.ModelManuValue = ((Excel.Range)range.Cells[i, k]).Text;
                                    }
                                    else
                                    {
                                        impItem.MiscId = GetAttributeLookup_Id(db, ((Excel.Range)range.Cells[i, k]).Text.Trim(), 23);
        ...
        protected string LogMessage(ImportedItem mfn, string oldStr, string newStr)
        {
            if (oldStr.Length == 0)
                return mfn.MFG_Number + " " + newStr;
            else
                return oldStr + "<br />" + mfn.MFG_Number + " " + newStr;
        }

        protected decimal CheckAndAsignValue(string input)
        {
            Regex reNum = new Regex(@"^(-?[0-9]+(\.[0-9]+)?)");   //(\\+|-)?([0-9]+(\\.[0-9]+))  ^\d+$
            ...
            else
            {
                ...
                else if (!reNum.Match(input).Success)
                {
                    input = "0.0";
                };
            }
        ...
        protected int GetManufacturerLogo_Id(IMS_V1Entities db, string abbr)
        {
            int manufacturerLogo_Id = 0;
            var m = db.zManufacturersLogoes.Where(z => z.Abbrev == abbr).FirstOrDefault();
            if (m != null)
                manufacturerLogo_Id = m.ManufacturerLogo_Id;
            return manufacturerLogo_Id;
        }


        protected int GetCategoryClass_Id(IMS_V1Entities db, string cid)
        {
            int categoryId = 0;
            var category = db.CategoryClasses.Where(c => c.Category_Id == cid).FirstOrDefault();
            if (category != null)
                categoryId = category.CategoryClass_Id;
            return categoryId;
        }

        protected int GetSubClass_Id(IMS_V1Entities db, string sid, string cid)
        {
            int subClassId = 0;
            var subclass = db.SubClasses.Where(sub => sub.SubClass_Id == sid && sub.Category_Id == cid).FirstOrDefault();
            if (subclass != null)
                subClassId = subclass.SubClassCode_Id;
            return subClassId;

        }

        protected int GetFineLineClass_Id(IMS_V1Entities db, string fid, string cid, string sid)
        {
            int fineLineClassId = 0;
            var fineline = db.FineLineClasses.Where(f => f.FineLine_Id == fid && f.Category_Id == cid && f.SubClass_id == sid).FirstOrDefault();
            if (fineline != null)
                fineLineClassId = fineline.FineLineCode_Id;
            return fineLineClassId;

        }

        protected int GetFFLType_Id(IMS_V1Entities db, string fflType)
        {
            int fflTypeId = 0;
            var ffl = db.FFLTypes.Where(f => f.FFLType_Description == fflType).FirstOrDefault();
            if (ffl != null)
                fflTypeId = ffl.FFLType_Id;
            return fflTypeId;
        }

        protected int GetCatWebCode_Id(IMS_V1Entities db, string webCode)
        {
            int catWebCodeId = 0;
            var webcd = db.WebCodes.Where(w => w.WebCode1 == webCode).FirstOrDefault();
            if (webcd != null)
                catWebCodeId = webcd.CatWebCode_Id;
            return catWebCodeId;
        }

        protected int GetFreight_Id(IMS_V1Entities db, string fCode)
        {
            int freightId = 0;
            var freight = db.Freight_Lookup.Where(f => f.Freight_APlusClass == fCode).FirstOrDefault();
            if (freight != null)
                freightId = freight.Freight_Id;
            return freightId;
        }

        protected int GetABC_Id(IMS_V1Entities db, string abc)
        {
            int abcId = 0;
            var abcLookup = db.ABC_Lookup.Where(a => a.ABC_Code == abc).FirstOrDefault();
            if (abcLookup != null)
                abcId = abcLookup.ABC_Id;
            return abcId;
        }

        protected int GetAttributeLookup_Id(IMS_V1Entities db, string webattr, int aTypeId)
        {
            int attributeLookup_Id = 0;
            var attributeLookup = db.Attribute_Lookup.Where(a => a.AttributeType_Id == aTypeId && a.WebsiteAttributeValue == webattr).FirstOrDefault();
            if (attributeLookup != null)
                attributeLookup_Id = attributeLookup.AttributeLookup_Id;
            return attributeLookup_Id;
        }
